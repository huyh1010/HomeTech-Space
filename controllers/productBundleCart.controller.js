...
const ProductBundleCart = require("../models/ProductBundleCart");
const User = require("../models/User");

const productBundleCartController = {};

productBundleCartController.addItemToCart = catchAsync(
  async (req, res, next) => {
    //get data
    ...
    //validation
    const cart = await ProductBundleCart.findOne({ user: userId });
    ...
    if (cart) {
      ...
      const newCart = await ProductBundleCart.create(cartData);
...
productBundleCartController.getUserCart = catchAsync(async (req, res, next) => {
  let userId = req.params.id;
  let user = await User.findById(userId);
  if (!user) throw new AppError(400, "Invalid User ID", "Get Cart Error");

  let cart = await ProductBundleCart.findOne({ user: userId }).populate("user");
  if (!cart) throw new AppError(400, "Cart not found", "Get Cart Error");

  sendResponse(res, 200, true, { cart }, null, "Get Cart Successful");
});

productBundleCartController.decreaseItemQuantity = catchAsync(
  async (req, res, next) => {
    let userId = req.params.id;
    ...
    let cart = await ProductBundleCart.findOne({ user: userId });
    if (!cart)
...
productBundleCartController.removeItemFromCart = catchAsync(
  async (req, res, next) => {
    let userId = req.params.id;
    let productId = req.body.product_id;
    let user = await User.findById(userId);
...
module.exports = productBundleCartController;
